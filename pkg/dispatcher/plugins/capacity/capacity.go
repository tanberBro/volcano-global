...
func (cp *capacityPlugin) buildQueueAttrs(ssn *framework.Session) {
	for _, queue := range ssn.Snapshot.QueueInfos {
		attr := &queueAttr{
			queueID:   queue.UID,
			name:      queue.Name,
		...
		if attr.capability == nil {
			...
			attr.capability = realCapability
		} else {
		...
		klog.V(4).Infof("Added Queue <%s> attributes.", queue.UID)
	}
	for _, rbi := range ssn.Snapshot.ResourceBindingInfos {
		if rbi.DispatchStatus == api.UnSuspended {
			if attr, ok := cp.queueOpts[volcanoapi.QueueID(ssn.GetResourceBindingInfoQueue(rbi))]; ok {
				attr.allocated = attr.allocated.Add(volcanoapi.NewResource(rbi.ResourceBinding.Spec.ReplicaRequirements.ResourceRequest).Multi(float64(rbi.ResourceBinding.Spec.Replicas)))
			} else {
...
func (cp *capacityPlugin) updateShare(attr *queueAttr) {
	...
	for _, rn := range attr.capability.ResourceNames() {
		res = max(res, helpers.Share(attr.allocated.Get(rn), attr.capability.Get(rn)))
	}
